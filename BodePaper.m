...
% DO NOT EDIT THIS FILE - JUST PUT IT IN THE SAME FOLDER AS THE "bodeplot.m" FILE
...
xlabel('Frequency (rad/s)- log scale','fontsize',22);
grid
...
if UseRad==0,
    ...
    set(h,'YTick',ytick,'fontsize',22);
    ylabel('Phase (degrees)','fontsize',22);
else
    %If radians, make ticks every 0.25 radians.
    ytick=ph_lo+(pi/4)*[0:(ph_hi-ph_lo)/(pi/4)];
    set(h,'YTick',ytick,'fontsize',12);
    %Set the tick labels to be fractions of pi.
    set(h,'YTickLabel',num2str(ytick'/pi),'fontsize',22);
    ylabel('Phase/\pi   (radians/\pi)','fontsize',22);
end